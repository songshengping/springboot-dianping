...
import com.song.springboot.food.mapper.DpUserMapper;
...
public class UserService {
    ...
    public UserDTO findUserById(Long id) throws GeneralException {
        DpUser user = userMapper.selectByPrimaryKey(id);
        if (user == null) {
    ...
    public Boolean userRegist(UserRegistRequest requestBody) {
        DpUser user = UserMapStruct.INSTANCE.toModel(requestBody);
    ...
    public Boolean findUserByUsername(String username) {
        List<DpUser> user = userMapper.findDpUserByUsername(username);
        return !user.isEmpty();
    }
}
